...
namespace Demo.Scripts.Runtime
{
    ...
    public class FPSController : FPSAnimController
    {
        ...
        [HideInInspector] public FPSActionState actionState;
        ...
        private void Fire()
        {
            ...
            if (UiManager.instance.crosshair != null) UiManager.instance.crosshair.Resize(GetGun().weaponAsset.crosshairSpread);
        ...
        private void Hit(LayerMask layer, float damage, RaycastHit h, bool damageTarget)
        {
            ...
            switch (layer)
            {
                case int l when l == LayerMask.NameToLayer("Ground"):
                    impact = Instantiate(groundImpactParticle, h.point, Quaternion.identity);
                ...
                case int l when l == LayerMask.NameToLayer("Grass"):
                    impact = Instantiate(grassImpactParticle, h.point, Quaternion.identity);
                    impact.transform.rotation = Quaternion.LookRotation(h.normal);
                    break;
                case int l when l == LayerMask.NameToLayer("Metal"):
                    impact = Instantiate(metalImpactParticle, h.point, Quaternion.identity);
                    impact.transform.rotation = Quaternion.LookRotation(h.normal);
                    impactBullet = Instantiate(ImpactHole, h.point, Quaternion.identity);
                    break;
            ...
            {
                Vector3 forceDirection = (h.point - transform.position).normalized;
                DamageBot(h.collider, damage, forceDirection * GetGun().weaponAsset.force, h.point);
            }
            if (h.collider.gameObject.CompareTag("BotHeadShot"))
            {
                Vector3 forceDirection = (h.point - transform.position).normalized;
                DamageBot(h.collider, damage * GetGun().weaponAsset.headDamageMultiply, forceDirection * GetGun().weaponAsset.force, h.point);
            }
            if (h.collider.gameObject.CompareTag("BotLimbShot"))
            {
                Vector3 forceDirection = (h.point - transform.position).normalized;
                DamageBot(h.collider, damage * GetGun().weaponAsset.limbDamageMultiply, forceDirection * GetGun().weaponAsset.force, h.point);
            }
        }
